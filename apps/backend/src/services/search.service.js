...
async function searchProductsAdvanced(params) {
  ...
  const [ratingsMap, shops, distanceMap] = await Promise.all([
    ...
    Shop.find({ _id: { $in: shopIds } })
      .select("name trustScore popularityScore")
      .lean(),
  ...
  const weighted = products.map((product) => {
    ...
    const distanceKm =
      distanceMap.get(String(product.shopId)) != null ? distanceMap.get(String(product.shopId)) : null;
  ...
  if (minDiscount != null)
    filtered = filtered.filter((row) => Number(row.discountRate || 0) >= minDiscount);
  if (maxDistance != null)
    filtered = filtered.filter((row) => row.distanceKm != null && row.distanceKm <= maxDistance);
...
async function searchShopsAdvanced(params) {
  ...
  let shops = await Shop.find(filter).select("name slug domain trustScore popularityScore").limit(limit).lean();
