...
async function evaluateTransaction({ orderId, paymentAttemptId = null, source = "manual_check", context = {} }) {
  ...
  const { order, paymentAttempts, paymentAttempt, merchantOrders, userOrders, couponCarts } =
  ...
  const distinctActorsOnDevice = getDistinctActorsOnDevice(recentAuditSignals, normalizedContext.deviceHash);
  ...
  score += appendSignal(signals, userAgeHours < 24, { code: "NEW_ACCOUNT", label: "Very new account age", weight: 16, value: `${userAgeHours.toFixed(1)}h`, threshold: "<24h" });
  score += appendSignal(signals, recentUserOrders >= 4, { code: "RAPID_ORDERING", label: "Rapid ordering velocity", weight: Math.min(30, 10 + recentUserOrders * 4), value: recentUserOrders, threshold: ">=4 in 30m" });
  score += appendSignal(signals, failedAttempts >= 2, { code: "PAYMENT_FAILURES", label: "Multiple payment failures", weight: Math.min(24, failedAttempts * 8), value: failedAttempts, threshold: ">=2" });
  score += appendSignal(signals, order.paymentStatus === "FAILED" || order.status === "PAYMENT_FAILED", { code: "ORDER_PAYMENT_FAILED", label: "Current order has failed payment", weight: 22, value: order.paymentStatus || order.status, threshold: "FAILED" });
  score += appendSignal(signals, averagePreviousAmount > 0 && Number(order.totalAmount || 0) >= averagePreviousAmount * 2.5 && Number(order.totalAmount || 0) >= 3000, { code: "AMOUNT_SPIKE", label: "Order amount is significantly above normal", weight: 14, value: Number(order.totalAmount || 0), threshold: `${Math.round(averagePreviousAmount * 2.5)}` });
  score += appendSignal(signals, sameIpRecentActions >= 6, { code: "SAME_IP_BURST", label: "Heavy activity from the same IP", weight: 18, value: sameIpRecentActions, threshold: ">=6 in 10m" });
  score += appendSignal(signals, distinctActorsOnDevice >= 3, { code: "SHARED_DEVICE", label: "Device fingerprint reused across accounts", weight: 16, value: distinctActorsOnDevice, threshold: ">=3 accounts" });
  score += appendSignal(signals, refundedUserOrders >= 2, { code: "REFUND_HISTORY", label: "User has repeated refunds", weight: 12, value: refundedUserOrders, threshold: ">=2" });
  score += appendSignal(signals, merchantOrders.length >= 10 && merchantRefundRate >= 0.15, { code: "MERCHANT_REFUND_RATE", label: "Merchant refund rate is abnormally high", weight: 18, value: `${(merchantRefundRate * 100).toFixed(1)}%`, threshold: ">=15%" });
  score += appendSignal(signals, order.isGuest, { code: "GUEST_CHECKOUT", label: "Guest checkout without account history", weight: 8, value: true, threshold: "guest=true" });
  score += appendSignal(signals, sameContactOrders >= 4, { code: "CONTACT_REUSE", label: "Contact details reused in many orders", weight: 12, value: sameContactOrders, threshold: ">=4 orders" });
  score += appendSignal(signals, couponUsage >= 4, { code: "COUPON_REUSE", label: "Coupon used across multiple carts", weight: 15, value: couponUsage, threshold: ">=4 carts" });
  score += appendSignal(signals, attemptBurst >= 4, { code: "PAYMENT_ATTEMPT_BURST", label: "Too many attempts for one checkout", weight: 14, value: attemptBurst, threshold: ">=4 attempts" });
  score += appendSignal(signals, Boolean(paymentAttempt && Number(paymentAttempt.amount || 0) !== Number(order.totalAmount || 0)), { code: "PAYMENT_MISMATCH", label: "Payment amount does not match order value", weight: 18, value: Number(paymentAttempt?.amount || 0), threshold: Number(order.totalAmount || 0) });
  ...
  const recommendedActions = level === LEVELS.HIGH ? ["FLAG_TRANSACTION", "SEND_ADMIN_ALERT", "REQUIRE_MANUAL_REVIEW"] : level === LEVELS.MEDIUM ? ["FLAG_TRANSACTION", "SEND_ADMIN_ALERT"] : [];
  ...
  const shouldAlert =
    ...
    (!previousCase || previousCase.level !== level || Math.abs(Number(previousCase.score || 0) - score) >= 15);
...
async function getOverview() {
  ...
  const alerts = openCases.filter((item) => item.level === LEVELS.HIGH || item.level === LEVELS.MEDIUM).slice(0, 12);

  const suspiciousUsers = Array.from(orderCases.reduce((map, item) => {
    const key = String(item.userId || "");
    if (!key) return map;
    const current = map.get(key) || { userId: item.userId, score: 0, orderCount: 0, latestCaseAt: item.updatedAt };
  ...
  const highRiskMerchants = Array.from(orderCases.reduce((map, item) => {
    const key = String(item.shopId || "");
    if (!key) return map;
    const current = map.get(key) || { shopId: item.shopId, score: 0, caseCount: 0, latestCaseAt: item.updatedAt };
    current.score = Math.max(current.score, Number(item.score || 0));
    current.caseCount += 1;
    current.latestCaseAt = item.updatedAt > current.latestCaseAt ? item.updatedAt : current.latestCaseAt;
    map.set(key, current);
...
async function reviewCase({ caseId, action, note = "", adminId = null, req = null }) {
  ...
  if (action === "INVESTIGATE") fraudCase.status = "INVESTIGATING";
  ...
  return FraudCase.findOneAndUpdate(
    { caseKey },
    {
      $set: {
        ...
        metrics: { flagCount: combinedFlags.length, aiFraudScore: aiClaimSignals.score },
        lastEvaluatedAt: new Date(),
      },
