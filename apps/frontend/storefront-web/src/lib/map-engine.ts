...
export function clusterShops(shops: ShopMarker[], thresholdKm = 1.5) {
  const clusters: Cluster[] = [];

  shops.forEach((shop) => {
    const existing = clusters.find(
      (cluster) => getDistanceKm({ lat: cluster.lat, lng: cluster.lng }, shop) <= thresholdKm
    );
    if (!existing) {
      clusters.push({
...
export function getDistanceKm(from: Location, to: Location) {
...
export function computeBounds(shops: ShopMarker[]) {
  if (!shops.length) {
