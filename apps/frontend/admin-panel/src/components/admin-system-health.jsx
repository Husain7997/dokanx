...
export function AdminSystemHealth() {
    ...
    return (<div className="grid gap-3 rounded-3xl border border-white/40 bg-white/70 p-6 shadow-sm">
      <p className="text-sm font-semibold text-foreground">Platform health</p>
      {state.loading ? (<p className="text-sm text-muted-foreground">Checking core services...</p>) : state.error ? (<p className="text-sm text-red-600">{state.error}</p>) : (<div className="grid gap-2 text-sm text-muted-foreground">
