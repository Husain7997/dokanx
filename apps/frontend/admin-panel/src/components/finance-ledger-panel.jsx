...
export function FinanceLedgerPanel() {
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                if (!active)
                    return;
                setSettlements(Array.isArray(settlementResponse.data) ? settlementResponse.data : []);
    ...
    const filteredSettlements = useMemo(() => {
        return settlements.filter((row) => {
            ...
            if (fromDate) {
                ...
                if (!created || Number.isNaN(created.getTime()) || created < from)
                    return false;
            }
            if (toDate) {
                const created = row.createdAt ? new Date(row.createdAt) : null;
    ...
    return (<Card>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        <Input value={shopFilter} onChange={(event) => setShopFilter(event.target.value)} placeholder="Filter by shop ID"/>
        <SelectDropdown label="Status" value={statusFilter} onValueChange={setStatusFilter} options={[
            { label: "All", value: "ALL" },
            { label: "Pending", value: "PENDING" },
            { label: "Paid", value: "PAID" },
        ]}/>
        <SelectDropdown label="Preset range" value={rangePreset} onValueChange={(value) => {
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
          ...
          {!settlements.length ? (<p className="mt-3 text-sm text-muted-foreground">No settlements yet.</p>) : null}
        </div>
        <div className="grid gap-3 text-sm text-muted-foreground">
          <p className="text-sm font-semibold text-foreground">Payout alerts</p>
          {alerts.slice(0, 6).map((alert, index) => (<div key={index} className="rounded-2xl border border-border/60 px-4 py-3">
...
function inferPreset(fromDate, toDate) {
    if (!fromDate || !toDate)
        return null;
    ...
    if (iso(start) === iso(last7Start) && iso(end) === iso(today))
        return "LAST_7";
    const last30Start = new Date(today);
    last30Start.setDate(today.getDate() - 29);
    if (iso(start) === iso(last30Start) && iso(end) === iso(today))
        return "LAST_30";
    ...
    if (iso(start) === iso(weekStart) && iso(end) === iso(weekEnd))
        return "THIS_WEEK";
    const monthStart = new Date(today.getFullYear(), today.getMonth(), 1);
    const monthEnd = new Date(today.getFullYear(), today.getMonth() + 1, 0);
    if (iso(start) === iso(monthStart) && iso(end) === iso(monthEnd))
        return "THIS_MONTH";
    ...
    if (iso(start) === iso(lastQuarterStart) && iso(end) === iso(lastQuarterEnd))
        return "LAST_QUARTER";
    const lastYearStart = new Date(today.getFullYear() - 1, 0, 1);
    const lastYearEnd = new Date(today.getFullYear() - 1, 11, 31);
    if (iso(start) === iso(lastYearStart) && iso(end) === iso(lastYearEnd))
        return "LAST_YEAR";
    return null;
}
