...
export function AdminFinanceOverview() {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                ...
                if (!active)
                    return;
                const series = seriesResponse.data?.map((item) => ({
    ...
    return (<div className="grid gap-4 rounded-3xl border border-white/40 bg-white/70 p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        {state.loading ? (<span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">
            Loading
          </span>) : state.error ? (<span className="rounded-full bg-red-100 px-3 py-1 text-xs font-semibold text-red-700">
            {state.error}
      ...
      <div className="grid gap-2 rounded-2xl border border-white/40 bg-white/80 p-4">
        ...
        {latest ? (<div className="flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground">
            <span>{latest.day}</span>
            <span>Revenue {latest.revenue} BDT</span>
            <span>Payout {latest.payout} BDT</span>
          </div>) : (<p className="text-sm text-muted-foreground">No settlement data yet.</p>)}
      </div>
